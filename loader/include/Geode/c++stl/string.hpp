...
#elif defined(FREOD_IS_MACOS) || defined(FREOD_IS_ANDROID)
	struct StringData {
		struct Internal {
			size_t m_size;
			size_t m_capacity;
...
namespace gd {
#if defined(FREOD_IS_MACOS) || defined(FREOD_IS_WINDOWS) || defined(FREOD_IS_IOS)
	...
	class FREOD_DLL string {
		freod::stl::StringData m_data;
		friend freod::stl::StringImpl;
	public:
		string();
		string(string const&);
		// string(string&&);
