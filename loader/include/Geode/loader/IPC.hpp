...
namespace freod::ipc {
    ...
    constexpr char const* IPC_PIPE_NAME = R"(\\.\pipe\FreodIPCPipe)";
    #endif

    #ifdef FREOD_IS_MACOS
    constexpr char const* IPC_PORT_NAME = "FreodIPCPipe";
    #endif

    class IPCFilter;

    // IPC (Inter-Process Communication) provides a way for Freod mods to talk
    ...
    // example, an external application can query what mods are loaded in Freod
    // by sending the `list-mods` message to `freod.loader`.

    class FREOD_DLL IPCEvent final : public Event {
    protected:
    ...
    class FREOD_DLL IPCFilter final : public EventFilter<IPCEvent> {
    public:
