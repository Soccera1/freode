...
static std::wstring getErrorString(DWORD error) {
    return L"Could not load Freod! Error code: " + std::to_wstring(error);
}

static DWORD errorThread(LPVOID param) {
    constexpr wchar_t REDIST_ERROR[] = L"Could not load Freod!\n"
    ...
    constexpr wchar_t ALT_REDIST_ERROR[] = L"Could not load Freod!\n\n"
...
BOOL WINAPI DllMain(HINSTANCE module, DWORD reason, LPVOID _) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        // This is UB.
        if (LoadLibraryW(L"Freod.dll") == NULL) {
