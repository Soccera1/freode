...
std::filesystem::path freodDir;
std::filesystem::path updatesDir;
std::filesystem::path resourcesDir;

void showError(std::string const& error) {
    MessageBoxA(nullptr, error.c_str(), "Error Loading Freod", MB_ICONERROR);
}
...
bool waitForFile(std::filesystem::path const& path) {
    ...
    if (hFile) {
        ...
        showError("Unable to update Freod: " + path.filename().string() + " is open by another process.");
        return false;
    }
    return true;
}
...
bool updateFile(std::string const& name) {
    ...
    if (error) {
        showError("Unable to update Freod: Unable to move " + name + " - " + error.message());
        return false;
    }
    return true;
}
...
void removePath(std::filesystem::path const& path) {
    ...
    if (error) {
        if (path.has_filename())
            showError("Unable to update Freod: Unable to remove " + path.filename().string() + " - " + error.message());
        else
            showError("Unable to update Freod: Unable to remove " + path.string() + " - " + error.message());
        return;
    }
}
...
void updateResources() {
    ...
    if (error) {
        showError("Unable to update Freod resources:" + error.message());
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    freodDir = workingDir / "freod";
    updatesDir = freodDir / "update";
    resourcesDir = freodDir / "resources";

    if (std::filesystem::exists(workingDir / "FreodBootstrapper.dll"))
        removePath(workingDir / "FreodBootstrapper.dll");

    if (std::filesystem::exists(freodDir) && std::filesystem::exists(updatesDir)) {
        bool updateSuccess = true;
        updateSuccess &= updateFile("XInput1_4.dll");
        updateSuccess &= updateFile("Freod.dll");
        updateSuccess &= updateFile("Freod.pdb");
        updateResources();
