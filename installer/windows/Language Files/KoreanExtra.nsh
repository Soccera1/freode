!insertmacro LANGFILE_EXT Korean

!pragma warning disable 6030
${LangFileString} MUI_TEXT_WELCOME_INFO_TEXT "이 프로그램은 $(^NameDA)의 설치를 진행해줄 것입니다.$\r$\n$\r$\n설치를 시작하기 전에, Geometry Dash가 켜져있지 않은지 확인해주세요.$\r$\n$\r$\n$_CLICK"
${LangFileString} MUI_UNTEXT_WELCOME_INFO_TEXT "이 프로그램은 $(^NameDA)의 삭제를 진행해줄 것입니다.$\r$\n$\r$\n삭제를 시작하기 전에, Geometry Dash가 켜져있지 않은지 확인해주세요.$\r$\n$\r$\n$_CLICK"
!pragma warning default 6030

; installer

${LangFileString} FREOD_TEXT_GD_MISSING "$\r$\n$\r$\n이 경로에는 Geometry Dash가 설치되어 있지 않습니다."
${LangFileString} FREOD_TEXT_GD_OLD "$\r$\n$\r$\nYour version of Geometry Dash is too old for this version of Freod!"
${LangFileString} FREOD_TEXT_MOD_LOADER_ALREADY_INSTALLED "이 경로에는 다른 모드가 설치되어 있습니다!$\r$\nFreod에 의해 덮어쓰게 될 것입니다. (the dll trademark)"

; uninstaller

${LangFileString} FREOD_UNTEXT_FREOD_MISSING "이 경로에는 Freod가 설치되어 있지 않습니다!"
